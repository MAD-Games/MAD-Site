...
USER_WHITELIST = ["frankvillle", "kitkatnik", "calipercrow", "cmykatze", "tanooki_the_artist", "chronorevolution", "goodwargames"]

get '/home' do
	client = Instagram.client(:access_token => session[:access_token])
  html = ""
  count = 0
  for media_item in (client.tag_recent_media('madgamestudios') + client.tag_recent_media('madgames'))
  	(USER_WHITELIST.include?(media_item["user"]["username"]) && count < 6 ) ? (html << "<a href='#{media_item.link}'><img src='#{media_item.images.low_resolution.url}'></a>"; (count+=1)) : nil
  end
	erb :splash, locals: {
		html: html
	}
...
Instagram.configure do |config|
  config.client_id = "0e044e7c69a349988ab281b3511cfb5e"
  config.client_secret = "735e8aebfa3c467fb8908c1f633060eb"
